...
cron.schedule("00 44 10 * * *", () => {}); 不支持异步 */

let timer;
let interval_mins = 1; // 1-59(性能与准确反之)
let target_HOURS = 12; // 整点

timer = setInterval(async function () {
  if (new Date().getHours() !== target_HOURS) {
    console.info("时间未到，忽略");
    return;
  }
  clearInterval(timer);
  console.time("耗时");
