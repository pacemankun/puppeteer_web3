...
cron.schedule("15 50 15 * * *", async function () {
  ...
  for (const item of mapUserId) {
    try {
      ...
      console.info(
        chalk.green(`stash3 >>>>>> filtered girlIds done:${girlIds}`)
      );
      await page2.close();
      ...
      if (!page4.url().includes("10")) {
        console.count(chalk.green("last page redirected count:"));
        await page4.goBack();
      ...
      console.info("account-error捕获xxxxxxxxxxxxxxxxxxxxx", item, error);
      continue;
    }
  }
  console.timeEnd("总耗时");
